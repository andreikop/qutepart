  ...
  while (true)
  {
    switch (modification.type)
    {
      ...
      case KateHlContextModification::doNothing:
        return getContextObjectByIndex (contextStack.isEmpty() ? 0 : contextStack.last());
      ...
      case KateHlContextModification::doPush:
        ...
        return getContextObjectByIndex (modification.newContext);
      ...
      default:
      {
        ...
        if (previousLineContextIndexInTheStack >= (contextStack.size()-1))
        {
          ...
          if ( contextStack.isEmpty() )
            return getContextObjectByIndex (0);

          KateHlContext *c = getContextObjectByIndex(contextStack.last());
 ...
 *
 * @param prevLineObject The previous line, the context array is picked up from that if present.
 ...
 */
void KateHighlighting::doHighlight ( Kate::TextLineData *prevLineObject,
                                     ...
                                     bool &ctxChanged )
{
  ...
  if (vectorOfContextIndexes.isEmpty())
  {
    ...
    currentLineContextObject = getContextObjectByIndex(0);
  }
  else
  {
    //if (lineContinue)   kDebug(13010)<<QString("The old context should be %1").arg((int)ctxNum);
    currentLineContextObject = getContextObjectByIndex(vectorOfContextIndexes.last());
  ...
  QChar lastCharValue = ' ';
  const QString& currentLineText = textLine->string();
  const int currentLineLength = textLine->length();

  // calc at which char the first char occurs, set it to length of line if never
  const int firstChar = textLine->firstChar();
  const int startNonSpace = (firstChar == -1) ? currentLineLength : firstChar;
  ...
  KateHighlighting::HighlightPropertyBag* additionalData = m_additionalData[currentLineContextObject->hlId];
  KateHlContext* oldContext = currentLineContextObject;

  // optimization: list of highlighting items that need their cache reset
  static QVarLengthArray<KateHlItem*> cachingItems;


  QChar lastDelimCharValue = 0;
  while (currentCharIndex < currentLineLength)
  {
    bool anItemMatched = false;
    bool customStartEnableDetermined = false;

    foreach (item, currentLineContextObject->items)
    {
      // does we only match if we are firstNonSpace?
      if (item->firstNonSpace && (currentCharIndex > startNonSpace))
        continue;

      // have we a column specified? if yes, only match at this column
      if ((item->column != -1) && (item->column != currentCharIndex))
        continue;
      ...
      if (!item->alwaysStartEnable)
      {
        ...
        {
            if ( oldContext != currentLineContextObject ) {
              oldContext = currentLineContextObject;
              additionalData = m_additionalData[oldContext->hlId];
            }
            if (customStartEnableDetermined || additionalData->deliminator.contains(lastCharValue))
        ...
        {
          if (lastDelimCharValue == lastCharValue) {
          } else if ( stdDeliminator.contains(lastCharValue) ) {
            lastDelimCharValue = lastCharValue;
          } else {
            continue;
          }
        }
      ...
      int matchEndIndex = item->tryFindThemselves(currentLineText, currentCharIndex, currentLineLength-currentCharIndex);
      if ( item->haveCache && !item->cachingHandled ) {
        cachingItems.append(item);
        item->cachingHandled = true;
      }

      if (matchEndIndex <= currentCharIndex)
        continue;
      ...
      if ( item->lookAhead && 
          ( item->ctx.pops < 2 && 
          item->ctx.newContext == ( vectorOfContextIndexes.isEmpty() ? 0 : vectorOfContextIndexes.last() ) ) )
        continue;
      ...
      if (item->region2)
      {
        ...
        else
        {
          ...
          if (item->region2<0) //check not really needed yet
            foldingList[foldingList.size()-1] = matchEndIndex;
          else
            foldingList[foldingList.size()-1] = currentCharIndex;
        }
      ...
      if (currentLineContextObject->dynamic)
      {
        ...
        if (!captures.empty())
        {
          ...
          int newctx = makeDynamicContext(currentLineContextObject, &captures);
          if (vectorOfContextIndexes.size() > 0)
            vectorOfContextIndexes[vectorOfContextIndexes.size() - 1] = newctx;

          currentLineContextObject = getContextObjectByIndex(newctx);
        }
      }

      // dominik: look ahead w/o changing currentCharIndex?
      if (!item->lookAhead)
      {
        if (matchEndIndex > currentLineLength)
          matchEndIndex = currentLineLength;

        // even set attributes ;)
        int attribute = item->onlyConsume ? currentLineContextObject->textFormat : item->textFormat;
        if (attribute > 0)
          textLine->setFormat (currentCharIndex, matchEndIndex-currentCharIndex, attribute);

        currentCharIndex = matchEndIndex;
        lastCharValue = currentLineText[currentCharIndex-1];
      }
    ...
    else
    {
      ...
      if (currentLineContextObject->textFormat > 0)
        textLine->setFormat (currentCharIndex, 1, currentLineContextObject->textFormat);

      lastCharValue = currentLineText[currentCharIndex];
      currentCharIndex++;
    }
  }
  ...
  if (m_foldingIndentationSensitive) {
    bool noindent=false;
    for(int i=vectorOfContextIndexes.size()-1; i>=0; --i) {
      if (getContextObjectByIndex(vectorOfContextIndexes[i])->noIndentationBasedFolding) {
        noindent=true;
        break;
      }
    }
                                               ...
                                               QStringList *ContextNameList)
{
  ...
  // dominik: look ahead and do not change currentCharIndex. so we can change contexts w/o changing offset1.
  ...
  {
    unresolvedContextReferences.insert(&(tmpItem->vectorOfContextIndexes),unresolvedContext);
  }
...
int KateHighlighting::attribute(int vectorOfContextIndexes) const
{
  return m_contexts[vectorOfContextIndexes]->attr;
}
  ...
  do
  {
    ...
    for (KateEmbeddedHlInfos::iterator it=embeddedHls.begin(); it!=embeddedHls.end();++it)
    {
      if (!it.value().loaded)  // we found one, we still have to load
      {
        ...
        //set class member for context 0 currentCharIndex, so we don't need to pass it around
...
void KateHighlighting::handleKateHlIncludeRulesRecursive(int index, KateHlIncludeRules *list)
{
  ...
  int index1 = index;
  int vectorOfContextIndexes = list->at(index1)->vectorOfContextIndexes;
  ...
  while (index < list->count() && list->at(index)->vectorOfContextIndexes == vectorOfContextIndexes)
  {
    index1 = index;
    ++index;
  }
  ...
  while (index1 >= 0 && index1 < list->count() && list->at(index1)->vectorOfContextIndexes == vectorOfContextIndexes)
  {
    ...
    {
      if (list->at(index2)->vectorOfContextIndexes == ctx1.newContext)
      {
    ...
    KateHlContext *dest=m_contexts[vectorOfContextIndexes];
    KateHlContext *src=m_contexts[ctx1.newContext];
//     kDebug(3010)<<"linking included rules from "<<vectorOfContextIndexes<<" to "<<ctx1;

    // If so desired, change the dest attribute to the one of the src.
    // Required to make commenting work, if currentLineText matched by the included context
